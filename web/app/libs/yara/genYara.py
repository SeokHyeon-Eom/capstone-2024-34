...
if __name__ == "__main__":
	...
	for label, idx in zip(cluster_labels, cluster_indices):
		...
		if label not in cluster_file:
			cluster_file[label] = []
		cluster_file[label].append(file_direc[idx].split('.')[1])

	
	file = list(cluster_file.values())
	...
	for signature_list in cluster_signatures.values():
		if signature_list == []:
			continue
		cluster_sig.append([i[0] for i in signature_list[: 50] if len(i[0]) <= 200])
	...
	json_data = json.dumps(
		{
			"rule": yara_rule, "extractSignature": signatures, 
   			"cluster_file": file, "cluster_sig": cluster_sig
		})
	print(json_data, end='')
